...
class ShoppingCartModel(object):
    ...
    def create_sales(self):
        ...
        try:
            ...
            query_to_add_shopping_cart = "INSERT INTO shopping_cart(users_id, products_id, quantity, unit_price, total_price) VALUES(%s,%s,%s,%s,%s)"
            connection.cursor.execute(query_to_add_shopping_cart,(self.current_user, self.products_id, change_type_quantity, change_type_unit_price, self.total_price))
            
            query_to_search_sale = "SELECT * FROM shopping_cart WHERE products_id=%s"
            ...
            print(exc)
    @staticmethod
    def create_many_sales(sales):
        """
        Adds many sales as an object to list
        :return: the sale that has just been added
        """
        try:
            many_sales = tuple(sales)
            cursor = connection.cursor()
            cursor.executemany("""INSERT INTO sales(users_id, products_id, quantity, unit_price, total_price) VALUES (%(users_id)s, %(products_id)s, %(quantity)s, %(unit_price)s, %(products_id)s)""", many_sales)
    ...
    def get_sales(cls):
        ...
        """
        response =[]
        query_to_get_all_sales = "select * from shopping_cart"
